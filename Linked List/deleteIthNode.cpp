//delete ith node of linked list
...
Node * deleteNode(Node *head, int i)
{
	...
	if(i == 0)            //to delete first node of ll
	{
		temp = temp -> next;
	...
	while(temp != NULL && count <i -1)      //to avoid traversing greater than the length of linked list
	{
		temp = temp -> next;    //pointing to i - 1 node
		count++;
	}
	
	if(temp != NULL)
	{
		Node *a = temp -> next;      //creating new nod to delete it
		temp -> next = a -> next;     //linking the current node to next node of deleted one
		delete a;                   //deallocation of unused memory
	}
